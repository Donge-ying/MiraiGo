...
import (
	...
	"encoding/hex"
	"fmt"
	"github.com/Donge-ying/MiraiGo/client/pb/qweb"
	"github.com/golang/protobuf/proto"
	"math/rand"
	"time"

	"github.com/Donge-ying/MiraiGo/binary"
	"github.com/Donge-ying/MiraiGo/binary/jce"
	"github.com/Donge-ying/MiraiGo/client/pb"
	"github.com/Donge-ying/MiraiGo/client/pb/cmd0x352"
	"github.com/Donge-ying/MiraiGo/client/pb/msg"
	"github.com/Donge-ying/MiraiGo/client/pb/oidb"
	"github.com/Donge-ying/MiraiGo/client/pb/pttcenter"
	"github.com/Donge-ying/MiraiGo/client/pb/structmsg"
	"github.com/Donge-ying/MiraiGo/message"
	"github.com/Donge-ying/MiraiGo/protocol/crypto"
	"github.com/Donge-ying/MiraiGo/protocol/packets"
	"github.com/Donge-ying/MiraiGo/protocol/tlv"
)
...
func (c *QQClient) buildLoginPacket() (uint16, []byte) {
	...
	sso := packets.BuildSsoPacket(seq, c.version.AppId, "wtlogin.login", c.DeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, req, c.ksid)
...
func (c *QQClient) buildDeviceLockLoginPacket(t402 []byte) (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x0810, crypto.ECDH, c.RandomKey, func(w *binary.Writer) {
		...
		w.Write(tlv.T116(c.version.MiscBitmap, c.version.SubSigmap))
		h := md5.Sum(append(append(c.DeviceInfo.Guid, []byte("stMNokHgxZUGhsYp")...), t402...))
		w.Write(tlv.T401(h[:]))
	})
	sso := packets.BuildSsoPacket(seq, c.version.AppId, "wtlogin.login", c.DeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, req, c.ksid)
	packet := packets.BuildLoginPacket(c.Uin, 2, make([]byte, 16), sso, []byte{})
...
func (c *QQClient) buildSMSCodeSubmitPacket(code string) (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x810, crypto.ECDH, c.RandomKey, func(w *binary.Writer) {
		...
		h := md5.Sum(append(append(c.DeviceInfo.Guid, []byte("12 34567890123456")...), c.t402...))
		w.Write(tlv.T401(h[:]))
		w.Write(tlv.T198())
	})
	sso := packets.BuildSsoPacket(seq, c.version.AppId, "wtlogin.login", c.DeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, req, c.ksid)
...
func (c *QQClient) buildRequestTgtgtNopicsigPacket() (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x0810, crypto.NewEncryptSession(c.sigInfo.t133), c.sigInfo.wtSessionTicketKey, func(w *binary.Writer) {
		...
		w.Write(tlv.T18(16, uint32(c.Uin)))
		w.Write(tlv.T1(uint32(c.Uin), c.DeviceInfo.IpAddress))
		w.Write(tlv.T106(uint32(c.Uin), 0, c.version.AppId, c.version.SSOVersion, c.PasswordMd5, true, c.DeviceInfo.Guid, c.DeviceInfo.TgtgtKey, 1))
		w.Write(tlv.T116(c.version.MiscBitmap, c.version.SubSigmap))
...
func (c *QQClient) buildClientRegisterPacket() (uint16, []byte) {
	...
	svc := &jce.SvcReqRegister{
		...
		CPID:         0,
		VendorName:   "MIUI",
		VendorOSName: string(c.DeviceInfo.Product),
	...
	sso := packets.BuildSsoPacket(seq, c.version.AppId, "StatSvc.register", c.DeviceInfo.IMEI, c.sigInfo.tgt, c.OutGoingPacketSessionId, pkt.ToBytes(), c.ksid)
