...
import (
	...
	devinfo "github.com/Donge-ying/MiraiGo/client/pb"
	...
	"github.com/Donge-ying/MiraiGo/utils"
...
func GenRandomDevice(p ClientProtocol) *DeviceInfo {
	...
	deviceInfo.Init()
	deviceInfo.Protocol = p
	r := make([]byte, 16)
	rand.Read(r)
	deviceInfo.Display = []byte("EPCLOUD." + utils.RandomStringRange(6, NumberRange) + ".001")
	deviceInfo.FingerPrint = []byte("sunfase/epcloud/epcloud:10/EPCLOUD.200122.001/" + utils.RandomStringRange(7, NumberRange) + ":user/release-keys")
	deviceInfo.BootId = []byte(binary.GenUUID(r))
	deviceInfo.ProcVersion = []byte("Linux version 3.0.31-" + utils.RandomString(8) + " (android-build@xxx.xxx.xxx.xxx.com)")
	rand.Read(r)
	t := md5.Sum(r)
	deviceInfo.IMSIMd5 = t[:]
	deviceInfo.IMEI = utils.RandomStringRange(15, NumberRange)
	deviceInfo.AndroidId = deviceInfo.Display
	deviceInfo.GenNewGuid()
	deviceInfo.GenNewTgtgtKey()
	return deviceInfo
}
...
func (info *DeviceInfo) Init() {
	*info = DeviceInfo{
		Display:     []byte("EPCLOUD.123456.001"),
		Product:     []byte("epcloud"),
		Device:      []byte("epcloud"),
		Board:       []byte("epcloud"),
		Brand:       []byte("sunfase"),
		Model:       []byte("epcloud"),
		Bootloader:  []byte("unknown"),
		FingerPrint: []byte("sunfase/epcloud/epcloud:10/EPCLOUD.200122.001/1234567:user/release-keys"),
		...
		AndroidId:   []byte("EPCLOUD.123456.001"),
		APN:         []byte("wifi"),
		Protocol:    IPad,
		Version: &Version{
...
func (c *QQClient) getSSOAddress() ([]*net.TCPAddr, error) {
	protocol := genVersionInfo(c.DeviceInfo.Protocol)
	...
	payload := jce.NewJceWriter(). // see ServerConfig.d
					...
					WriteInt32(int32(protocol.AppId), 6).WriteString(c.DeviceInfo.IMEI, 7).
