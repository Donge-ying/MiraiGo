...
func NewClient(uin int64, password string, info *DeviceInfo) *QQClient {
	return NewClientMd5(uin, md5.Sum([]byte(password)), info)
}
...
func NewClientMd5(uin int64, passwordMd5 [16]byte, info *DeviceInfo) *QQClient {
	cli := &QQClient{
		...
		ksid:                    []byte(fmt.Sprintf("|%s|A8.2.7.27f6ea96", info.IMEI)),
		eventHandlers:           &eventHandlers{},
		msgSvcCache:             utils.NewCache(time.Second * 15),
		transCache:              utils.NewCache(time.Second * 15),
		onlinePushCache:         utils.NewCache(time.Second * 15),
		version:                 genVersionInfo(info.Protocol),
		servers:                 []*net.TCPAddr{},
		stat:                    &Statistics{},
	}
	cli.DeviceInfo = info
	sso, err := cli.getSSOAddress()
...
/*func (c *QQClient) Init() {
	c.ksid = []byte(fmt.Sprintf("|%s|A8.2.7.27f6ea96", SystemDeviceInfo.IMEI))
	c.version = genVersionInfo(SystemDeviceInfo.Protocol)
}*/
...
func (c *QQClient) GetQzoneKeys() QzoneKeys {
	pskeys := make(map[string]string)
	for k, v := range c.sigInfo.psKeyMap {
		pskeys[k] = string(v)
	}
	return QzoneKeys{
		Pskeys:   pskeys,
		Skey:     c.getSKey(),
		Superkey: string(c.sigInfo.superkey),
		Uin:      c.Uin,
	}
}
...
func (c *QQClient) doHeartbeat() {
	...
	for c.Online {
		seq := c.nextSeq()
		sso := packets.BuildSsoPacket(seq, c.version.AppId, "Heartbeat.Alive", c.DeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, []byte{}, c.ksid)
